function Get-OSArchitecture {
    ...
    if($osAndArchitecture -eq "windows_arm64") {
        Write-ToConsoleLog "Windows arm64 is not currently supported by Terraform, so we will pull the Windows amd64 verison instead and run in emulation mode: https://learn.microsoft.com/en-us/windows/arm/apps-on-arm-x86-emulation" -IsWarning
        $architecture = "amd64"
